...
void i2c_init_config(uint8_t scl, uint8_t sda, uint32_t addr) {
		...
		i2c_config.freq=I2C_100KPBS;
		i2c_config.I2C_bus=NRF_TWI0;
    i2c_config.scl = scl;
    i2c_config.sda = sda;
...
//        {
//            // Do nothing.
//									terminalOut(" prvi error %d %d %d",i2c_config.I2C_bus->EVENTS_TXDSENT ,i2c_config.I2C_bus->EVENTS_ERROR,timeout );

//        }

//        if (timeout == 0 || i2c_config.I2C_bus->EVENTS_ERROR != 0)
//        {
//					terminalOut(" drugi error %d %d",timeout,i2c_config.I2C_bus->EVENTS_ERROR);
//					i2c_config.I2C_bus->EVENTS_ERROR = 0;
