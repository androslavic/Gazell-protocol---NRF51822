...
void
RewindIndexAtomic(volatile unsigned long *pulVal, unsigned long ulDelta)
{
	uint32_t irq_masked = __disable_irq();
   // bool bIntsOff;
                        ...
                        unsigned long ulLength)
{
    ...
    if((RingBufFree(ptRingBuf) > (ulLength+1)) && (ulLength < 0xFF) )
    {
        temp_length = (unsigned char)(ulLength & 0x000000FF);
        RingBufWriteOne(ptRingBuf, temp_length);
        RingBufWrite(ptRingBuf, pucData, ulLength);
    }
    else
    {
        //error
...
signed char RingBufQueueRead(tRingBufObject *ptRingBuf, unsigned char *pucData,
                        unsigned char * ulLength)
{
    unsigned char temp_data=0;

    temp_data = RingBufReadOne(ptRingBuf);
    if(RingBufUsed(ptRingBuf) >= temp_data)
    {
        RingBufRead(ptRingBuf, pucData, temp_data);
    }
    else
    {
        //error, flush
